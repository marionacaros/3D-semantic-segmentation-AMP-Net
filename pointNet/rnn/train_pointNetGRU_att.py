...
def train_gru(task: str,
    ...
    train_dataset = LidarDataset(dataset_folder=dataset_folder,
                                 ...
                                 c_sample=False,
                                 path_kmeans='/dades/LIDAR/towers_detection/datasets/pc_kmeans')
    ...
    for epoch in progressbar(range(epochs), redirect_stdout=True):
        ...
        for data in train_dataloader:
            ...
            if task == 'segmentation':
                iou_tower = get_iou_tower(targets, pc_preds)
...
def train_loop(data, optimizer_rnn, optimizer_att, ce_loss, rnn_pointnet, attn_model, n_points=2048,
    ...
    """
    metrics = {}
    pc_w, pc, targets, filenames = data
    ...
    pc_w, pc, targets = pc_w.to(device), pc.to(device), targets.to(device)  # [b, 2048, dims, w_len], [b, w_len]
    ...
    pc_hiddens = []
    pc_local_features = []
    # GRU through all windows
    for w in range(pc_w.shape[3]):
        in_points = pc_w[:, :, :, w]  # get window
        out_h, feat_transf = rnn_pointnet(in_points, hidden, get_local_feat=False)
        # [batch, 512] [b,64,64] [b,n_p,64]
        pc_hiddens.append(out_h)

    global_embeds = torch.stack(pc_hiddens, dim=1).to(device)  # [b, seq_len, 512]
    seq_len = global_embeds.shape[1]
    # reshape needed to input tensor into attn_model
    global_embeds = global_embeds.view(seq_len, batch_size, -1)  # [seq_len, b, 512]
    ...
    if task == 'segmentation':
        local_features, feat_transf = rnn_pointnet(pc, hidden, get_local_feat=True)  # [points, b, 64], [points, 64, 64]

        # pc_hiddens_repeat = global_embeds.view(-1, seq_len, global_embeds.shape[2], 1).repeat(1, 1, 1, n_points)  # [b, seq_len, 512, 2048]
        # pc_local_features = torch.stack(pc_local_features, dim=2).to(device)  # [b, 2048, w_len, 64]
        # pc_local_features = pc_local_features.view(-1, seq_len, local_features.shape[2], n_points)  # [b, seq_len, 64, 2048]
        # global_embeds = torch.cat((pc_hiddens_repeat, pc_local_features), dim=2)  # [b, w_len, 576, 2048]
        logits, att_weights = attn_model(global_embeds, local_features, attn_mask=None)

    else:  # classification
        logits, att_weights = attn_model(global_embeds, attn_mask=None)
...
def get_iou_tower(preds, targets_pc):
    pc_preds = preds.view(-1)
    targets = targets_pc.view(-1)
    # get metrics
